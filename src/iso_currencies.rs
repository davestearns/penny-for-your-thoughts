...
/// Returns a reference to the [Currency] with the provided `code`,
/// or `None` if the code did not match one of the ISO currencies.
pub fn find(code: &str) -> Option<&dyn Currency> {
    match code {
        "AFN" => Some(&AFN),
        "EUR" => Some(&EUR),
        "ALL" => Some(&ALL),
